...
type ThrouWatcherSync = (pathname: string) => void;
async function _throuSync(
  ...
  for (let i = 0; i < dirs.length; i++) {
    ...
    // Recursively resolve folders
    if (_isFolder) _throuSync(dir, watcher, observe);
  }
}
 ...
 * @default context Observes both files and folders
 */
export async function throuSync(
  context: {
    watcher: ThrouWatcherSync;
    observe?: ThrouObserve;
  },
  ...
  const observe = context.observe ? context.observe : "both";
  for (const folder of folders) {
    _throuSync(folder, context.watcher, observe);
  }
}
...
export function findSync(
  ...
  for (const extension of extensions) {
    ...
    if (fs.existsSync(posibility))
      return { extension, found: posibility, tries };
  }
